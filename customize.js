...
(function() {
  ...
  function handlePhotoSelect(e) {
    ...
    compressImage(file, 800, 800, 0.45).then(function(compressed) {
      photoData[key] = compressed;
      if (preview) {
      ...
      var removeBtn = document.getElementById('remove_' + key);
  ...
  // ─── Create Surprise (jsonblob.com storage) ───
  async function createSurprise() {
    if (!templateHTML) { alert('La plantilla aún se está cargando...'); return null; }
    var customized = customizeHTML();

    var publishBtn = document.getElementById('btnPublish');
    if (publishBtn) {
      publishBtn.disabled = true;
      publishBtn.innerHTML = '<div class="spinner" style="width:18px;height:18px;border-width:2px;margin:0"></div> Subiendo sorpresa...';
    }

    try {
      // Store in jsonblob.com (free JSON storage, CORS enabled)
      ...
      if (!response.ok) {
        throw new Error('Error del servicio de almacenamiento (HTTP ' + response.status + ')');
      }

      // Get blob ID from Location header
      var location = response.headers.get('Location') || '';
      var blobId = location.split('/').pop();

      // Fallback: try to get from response JSON
      if (!blobId || blobId === 'jsonBlob') {
        try {
          var data = await response.json();
          if (data && data._id) blobId = data._id;
        } catch(e) {}
      }
      ...
      if (!blobId) {
        ...
        try {
          ...
          if (json._id) blobId = json._id;
        } catch(e) {}
      }

      if (!blobId) {
        throw new Error('No se pudo obtener el ID de la sorpresa');
      }

      // Build short URL
      var baseUrl = window.location.href.substring(0, window.location.href.lastIndexOf('/') + 1);
      var url = baseUrl + 'surprise.html?s=' + blobId;

      var fakeToken = blobId.substring(0, 12);
